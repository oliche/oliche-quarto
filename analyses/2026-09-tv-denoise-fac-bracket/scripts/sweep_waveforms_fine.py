"""Finer TV-denoise `fac` bracket for the `waveforms` feature group.

Follow-up to sweep_tv_denoise_fac.py: that coarse OFAT sweep showed raw_ap,
raw_lf and raw_lf_csd peak near the current default fac=1, while waveforms kept
improving past it (still rising at fac=10). Here raw_ap/raw_lf/raw_lf_csd are
held fixed at their own sweep-selected fac=0.1, and waveforms is swept over a
finer, wider grid to locate its actual optimum. Same fixed train/test PID split
as the coarse sweep (same VINTAGE, RANDOM_SEED, TEST_FRACTION).
"""
...
OTHER_FAC = 0.1  # raw_ap, raw_lf, raw_lf_csd held fixed here
WAVEFORMS_FAC_GRID = [0.1, 0.3, 1, 2, 3, 4, 5, 7, 10, 15, 20, 30]
TEST_FRACTION = 0.2
RANDOM_SEED = 12345
...
path_csv = path_results.joinpath(f"sweep_results_waveforms_fine_{VINTAGE}.csv")
...
def record(fac_value, accuracy, elapsed):
    results.append(dict(waveforms_fac=fac_value, accuracy=accuracy, elapsed_s=elapsed))
    pd.DataFrame(results).to_csv(path_csv, index=False)
    print(f"waveforms_fac={fac_value:<6} accuracy={accuracy:.4f} ({elapsed:.0f}s)")


for fac_value in WAVEFORMS_FAC_GRID:
    fac = {
        "raw_ap": OTHER_FAC,
        "raw_lf": OTHER_FAC,
        "raw_lf_csd": OTHER_FAC,
        "waveforms": fac_value,
    }
    t0 = time.time()
    accuracy = evaluate(fac)
    record(fac_value, accuracy, time.time() - t0)

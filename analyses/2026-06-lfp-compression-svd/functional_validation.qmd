...
![Wheel-speed decoding R² per recording (uncompressed, mild, default, aggressive), vs shuffled null (black ticks).](figures/2026-09-05_lfp-compression-decoding_r2-by-pid.png){#fig-r2-by-pid .lightbox}

![Choice / feedback balanced accuracy per recording, one box per tier (uncompressed, mild, default, aggressive). Dashed line = chance.](figures/2026-09-05_lfp-compression-decoding_categorical_boxplot.png){#fig-categorical-boxplot .lightbox}
...
under `~/Documents/ibldevtools/olivier/` (sweep computation + this page's figures). The `mild`
tier was added after the fact by `2026-09-05_LFP_compression_decoding_add_mild.py` /
`2026-09-05_LFP_compression_decoding_categorical_add_mild.py` (reuse the already-computed
uncompressed/default/aggressive rows, decode only the new variant, regenerate @fig-r2-by-pid /
@fig-categorical-boxplot) and directly in
`2026-08-31_LFP_compression_full_grid_displays.py` (@fig-grid-heatmaps / @fig-grid-pareto
already covered the ε=100/α=14 cell — only the mild/default/aggressive highlight markers were
added), same directory.

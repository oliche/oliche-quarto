...
![Mean decode metrics and realized CR over the full (ε, α) grid.](figures/2026-09-01_grid_heatmaps.png){.lightbox}
...
vs. 140× and 135× in its immediate neighbors), the interaction in action.

![Decoding metrics grouped by epsilon (hue = alpha), plotted as each recording's own delta from its default (150,28) cell — this removes the ~0.11 SD between-recording offset that hides the effect in raw values.](figures/2026-09-01_grid_boxplots_by_epsilon.png){.lightbox}

![Total compressed size grouped by epsilon (hue = alpha) — the (450,96) bar is visibly shorter than its neighbors, again the interaction.](figures/2026-09-01_grid_barplot_size_by_epsilon.png){.lightbox}

![Curves per alpha vs. epsilon (thin = per-recording, heavy = median). Decoding curves are nearly flat against epsilon at any fixed alpha; only CR keeps climbing, and does so more steeply for higher alpha.](figures/2026-09-01_grid_curves_per_alpha.png){.lightbox}

![Pareto: realized CR vs. chance/zero-corrected retained decode fraction, one panel per target (marker size = alpha, colour = epsilon). Default (red star) sits on the frontier for wheel and near it for feedback.](figures/2026-09-01_grid_pareto.png){.lightbox}

...
benchmark recordings used throughout this page — 14.8 h of LFP in total.
...
![Mean decode metrics and realized CR over the full (ε, α) grid. Uncompressed is added as a literal (ε=0, α=0) corner cell; the rest of that extra row/column is left blank. Default and aggressive cells are outlined (red / orange).](figures/2026-09-01_grid_heatmaps.png){.lightbox}
...
![Total compressed size grouped by epsilon (hue = alpha), log scale, with the uncompressed total (20.4 GB, blue dash-dot line) for scale — every tested tier is at least ~40× smaller — and a secondary axis converting size directly to realized CR. The (450,96) bar is visibly shorter than its neighbors, again the interaction.](figures/2026-09-01_grid_barplot_size_by_epsilon.png){.lightbox}
...
::: {.column-page}
![Pareto: realized CR vs. retained decode fraction, one panel per target (marker size = alpha, colour = epsilon). Default (red star) sits on the frontier for wheel and near it for feedback.](figures/2026-09-01_grid_pareto.png){.lightbox}
:::
...
- **Update (2026-09-05): this became the `mild` tier.** ε=100, α=14 was named `mild` and shipped
  as a third production tier alongside default (aggressive retired per above), giving three
  named options instead of two: `mild` (ε=100, α=14, this row) for the highest fidelity,
  `default` (ε=150, α=28) as the standard, `aggressive` no longer recommended. Aggregated over
  the same 11 recordings: realized CR 48× (vs. default's 87×, uncompressed's 1×), feedback
  balanced accuracy 0.626 vs. default's 0.597 and uncompressed's 0.642 — a chance-corrected
  retained-feedback-fraction of **0.89** vs. default's 0.68. Confirmed at full ephys-atlas cohort
  scale (1099 PIDs, not just these 11) by an independent region-decoding validation — see
  [Region-decoding validation](region_decoding.qmd).

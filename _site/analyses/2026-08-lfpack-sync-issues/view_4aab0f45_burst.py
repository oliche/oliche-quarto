"""
Stream and display raw AP band around the 4aab0f45 shared cross-probe micro-burst
(nidq t~1020.92s), 1s padding on each side, for both probes, in viewephys.

Needs viewephys on the path, e.g. run with:
    PYTHONPATH=<repo>/packages/viewephys/src python view_4aab0f45_burst.py
"""
# %%
import json
...
PIDS = {
    'probe00': '2adc4f5d-bc7b-42a4-be76-f5df33d713d4',
    'probe01': '0d59e3a1-86c1-44bd-b291-d1f8bc8327ba',
}
T_EVENT = 1020.92  # s, nidq time of the burst (both probes' own front times track this closely)
...
# so forcing one gain across both washes out whichever one it wasn't computed from
ref_gain = {'raw': None, 'destriped': 0.0}
...
for probe, pid in PIDS.items():
    ...
    for tag, data in [('raw', raw), ('destriped', dst)]:
        ...
        ev = viewephys(data, sr.fs, channels=channels, title=title, t0=t0)
